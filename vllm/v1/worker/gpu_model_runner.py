...
class GPUModelRunner(
    ...
    def _dummy_sampler_run(
        ...
        try:
            warmup_dummy_metadata = dummy_metadata
            if (
                envs.VLLM_USE_FLASHINFER_SAMPLER
                and self.sampler.logprobs_mode not in (
                    "processed_logits",
                    "processed_logprobs",
                )
            ):
                # FlashInfer's top-k/top-p sampler can fail on synthetic
                # profiling logits on sm75. A per-request generator selects
                # the existing native sampler fallback for warmup only; real
                # requests without generators still use FlashInfer.
                warmup_dummy_metadata = replace(
                    dummy_metadata,
                    generators={
            ...
            sampler_output = self.sampler(
                logits=logits, sampling_metadata=warmup_dummy_metadata
            )
            ...
            if self.sampler.logprobs_mode not in (
                ...
                envs.VLLM_USE_FLASHINFER_SAMPLER
                and warmup_dummy_metadata.generators
            ):
                self.sampler(
                    logits=logits,
        ...
        def get_attn_backends_for_group(
            ...
            for layer_name in kv_cache_group_spec.layer_names:
                ...
                if isinstance(layer_kv_cache_spec, UniformTypeKVCacheSpecs):
                    specs = layer_kv_cache_spec.kv_cache_specs
                    if layer_name in specs:
                        layer_kv_cache_spec = specs[layer_name]
                    else:
                        shared_target = getattr(
                            layers[layer_name], "kv_sharing_target_layer_name", None
                        )
                        if shared_target is None or shared_target not in specs:
                            raise KeyError(layer_name)
                        layer_kv_cache_spec = specs[shared_target]
